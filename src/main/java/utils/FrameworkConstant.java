...
public final class FrameworkConstant {
    ...
    public static Path getConfigReaderFilePath() {
        ...
        if (Objects.nonNull(env)) {
            return Paths.get(getProjectPath(), "src", "main", "resources", "config", "config_" + env + ".properties");
        } else {
            return Paths.get(getProjectPath(), "src", "main", "resources", "config", "config.properties");
        }
    }
    ...
    public static Path getConfigYamlFilePath() {
        String env = Utilities.getEnv();
        if (Objects.nonNull(env)) {
            return Paths.get(getProjectPath(), "src", "main", "resources", "config", "config_" + env + ".yaml");
        } else {
            return Paths.get(getProjectPath(), "src", "main", "resources", "config", "config.yaml");
        }
    }
